    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto">
            ...
            <div class="mb-6">
                ...
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Track all transaction activities within your family account.</p>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg mb-6 border border-gray-200 dark:border-gray-700">
                <div class="p-6">
                    <form method="GET" action="{{ route('audit-logs.index') }}"
                        ...
                        {{-- Actor Filter --}}
                        <div>
                            <label for="user_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Performed
                                By</label>
                        ...
                        {{-- Start Date --}}
                        <div>
                            <label for="start_date" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">From
                                Date</label>
                        ...
                        <div class="flex items-end space-x-2">
                            ...
                            @if(request()->hasAny(['action', 'user_id', 'start_date', 'end_date']))
                                <a href="{{ route('audit-logs.index') }}"
                                    class="inline-flex items-center px-4 py-2 bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md font-semibold text-xs text-gray-700 dark:text-gray-200 uppercase tracking-widest hover:bg-gray-200 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150">
                                    Clear
                                </a>
                            @endif
                        </div>
                ...
                <div class="p-6">
                                        ...
                                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                            ...
                                            <td class="px-4 py-4 whitespace-nowrap">
                                                <div class="flex items-center">
                                                    <div
                                                        class="w-8 h-8 rounded-full bg-indigo-100 dark:bg-indigo-900/50 flex items-center justify-center text-indigo-600 dark:text-indigo-400 font-semibold text-sm">
                                            ...
                                            <td class="px-4 py-4 whitespace-nowrap text-sm">
                                                <div class="font-medium text-gray-900 dark:text-gray-100">{{ $log->model_name }}
                                                    #{{ $log->model_id }}</div>
                                                @if($log->targetUser)
                                                    <div class="text-gray-500 dark:text-gray-400 text-xs">by {{ $log->targetUser->name }}</div>
                                                @endif
                                            </td>
                                            <td class="px-4 py-4 text-sm text-gray-900 dark:text-gray-100">
                                                @if($log->action === 'updated' && $log->new_values)
                                                    <div x-data="{ open: false }" class="relative">
                                                        <button @click="open = !open" type="button"
                                                            class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-300 text-xs flex items-center">
                                                        ...
                                                        <div x-show="open" x-cloak
                                                            class="mt-2 bg-gray-50 dark:bg-gray-700 rounded-md p-3 text-xs max-w-md">
                                                            @foreach($log->new_values as $field => $newValue)
                                                                <div class="mb-2 last:mb-0">
                                                                    <span
                                                                        class="font-semibold text-gray-700 dark:text-gray-300">{{ ucfirst(str_replace('_', ' ', $field)) }}:</span>
                                                                    <div class="flex items-center mt-1">
                                                                        <span
                                                                            class="text-red-600 dark:text-red-400 line-through mr-2">{{ $log->old_values[$field] ?? 'N/A' }}</span>
                                                ...
                                                @elseif($log->action === 'created' && $log->new_values)
                                                    <div x-data="{ open: false }" class="relative">
                                                        ...
                                                        <div x-show="open" x-cloak
                                                            class="mt-2 bg-green-50 dark:bg-green-900/30 rounded-md p-3 text-xs max-w-md">
                                                                ...
                                                                @if(!in_array($field, ['id', 'tenant_id', 'user_id']))
                                                                    <div class="mb-1 last:mb-0">
                                                                        <span
                                                                            class="font-semibold text-gray-700 dark:text-gray-300">{{ ucfirst(str_replace('_', ' ', $field)) }}:</span>
                                                                        <span
                                                                            class="text-green-700 dark:text-green-400 ml-1">{{ is_array($value) ? json_encode($value) : $value }}</span>
                                                                    </div>
                                                                @endif
                                                            @endforeach
                                                        </div>
                                                ...
                                                @elseif($log->action === 'deleted' && $log->old_values)
                                                    <div x-data="{ open: false }" class="relative">
                                                        ...
                                                        <div x-show="open" x-cloak
                                                            class="mt-2 bg-red-50 dark:bg-red-900/30 rounded-md p-3 text-xs max-w-md">
                                                                ...
                                                                @if(!in_array($field, ['id', 'tenant_id', 'user_id']))
                                                                    <div class="mb-1 last:mb-0">
                                                                        <span
                                                                            class="font-semibold text-gray-700 dark:text-gray-300">{{ ucfirst(str_replace('_', ' ', $field)) }}:</span>
                                                                        <span
                                                                            class="text-red-700 dark:text-red-400 ml-1">{{ is_array($value) ? json_encode($value) : $value }}</span>
                                                                    </div>
                                                                @endif
                                                            @endforeach
                                                        </div>
                    ...
                    @else
                        <div class="text-center py-12">
                            ...
                            <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100">No audit logs found</h3>
